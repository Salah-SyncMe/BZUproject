...
import 'package:bzuappgraduation/model/post.dart';
...
class API extends ChangeNotifier {
  ...
  Future<void> createMyUsers() async {
    try {
      await firestore
          .collection('users')
          .doc(me?.email)
          ...
          .then((_) => printLog('Placeholder document added.'))
          .catchError(
              (error) => print('Error adding placeholder document: $error'));
  ...
  Future<void> createMyPages() async {
    try {
      await firestore
          .collection('users')
          .doc(me?.email)
  ...
  Future<ChatUser> getUser(String email) async {
    var data = await firestore.collection('users').doc(email.toString()).get();

    var user = (ChatUser.fromJson(data.data() ?? {}));
    return user;
  }

  Future<PageUser> getPage(String email, String name) async {
    var data = await firestore
        .collection('users/${email.toString()}/pages')
        .doc(name.toString())
        .get();

    var page = (PageUser.fromJson(data.data() ?? {}));
    return page;
  }
...
//
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> getAllCertainPostsFriend(
      ChatUser user) {
    return firestore
        .collection('users/${user.email}/posts')
        .orderBy('created_at', descending: true)
        .snapshots();
  }
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> getAllFriendsWithoutMe() {
    return firestore
        .collection('users')
        .where('email', isNotEqualTo: me?.email)
        .snapshots();
  ...
  Stream<List<ChatUser>> getFilteredFriends() {
    // Stream of all friends excluding the current user
    var allFriends = getAllFriendsWithoutMe().map((snapshot) =>
        snapshot.docs.map((doc) => ChatUser.fromJson(doc.data())).toList());

    // Stream of IDs from the 'myUsers' collection
    var addedFriendsIds = firestore
        .collection('users/${me?.email}/myUsers')
        .snapshots()
        .map((snapshot) => snapshot.docs
            .map((doc) => doc.id.toString().trim().toLowerCase())
            .toSet());
    ...
    // Combine and filter the lists
    return Rx.combineLatest2(allFriends, addedFriendsIds,
        (List<ChatUser> pages, Set<String> addedIds) {
      return pages
          // Filter out the friends who are already added
          .where((element) =>
              !addedIds.contains(element.email.toLowerCase().trim()))
          .toList();
    });
  }
  ...
  Future<void> deletePostPage(Post post) async {
    await firestore
        .collection('users/${me?.email}/pages/${post.name}/posts')
        .doc(
            '${post.createAt}_${me?.email.substring(0, me!.email.indexOf('@'))}')
        .delete();
    await firestore
...
//
  ...
  Stream<List<Map<String, dynamic>>> getPagesAllMeAndPostsPagesAndPostUser() {
    ...
    Stream<List<Map<String, dynamic>>> userSpecificPostsStream = firestore
        ...
        .map((snapshot) => snapshot.docs.map((doc) => doc.data()).toList())
        .doOnData((data) {
      printLog('User Specific Posts: ${data.length}');
    });

    Stream<List<String>> friendsEmailsStream = firestore
        .collection('users/${me?.email}/myUsers')
        .snapshots()
        .map((snapshot) => snapshot.docs.map((doc) => doc.id).toList());

    Stream<List<Map<String, dynamic>>> friendsPostsStream =
        friendsEmailsStream.flatMap((List<String> emails) {
      List<Stream<List<Map<String, dynamic>>>> postsStreams =
          emails.map((email) {
        return firestore
            .collection('postsUsers')
            .where('email', isEqualTo: email)
      ...
      return Rx.combineLatestList(postsStreams)
          .map((List<List<Map<String, dynamic>>> postsLists) {
        return postsLists.expand((element) => element).toList();
      });
    }).doOnData((data) {
      printLog('Friends Posts: ${data.length}');
    });
    ...
    return addedPageIdsStream.flatMap((List<String> pageIds) {
      if (pageIds.isEmpty) {
        return Rx.combineLatestList(
                [myPostsStream, userSpecificPostsStream, friendsPostsStream])
        ...
        List<Stream<List<Map<String, dynamic>>>> postsFromAddedPagesStreams =
          ...
          return firestore
              ...
              .doOnData((data) {
            printLog('Posts from Page ID $id: ${data.length}');
          });
        }).toList();
        ...
        return Rx.combineLatestList([
          ...
          var combinedList = allLists.expand((element) => element).toList();
          printLog('Total Posts after merge: ${combinedList.length}');
          return combinedList;
        });
      }
    });
  ...
  Future<void> addFriends(ChatUser page) async {
    try {
      printLog(page.id);
      await firestore
          .collection('users/${me?.email}/myUsers')
      ...
      await isAddedFriendBefore(page.id);
      notifyListeners();
    } on Exception catch (e) {
      printLog("error: $e");
    }
  ...
  Future<bool> isAddedFriendBefore(String id) async {
    try {
      final data = await firestore
          .collection('users/${me?.email}/myUsers')
          .where('id', isEqualTo: id)
  ...
  Future<void> unfollowFriend(ChatUser page) async {
    await firestore
        .collection('users/${me?.email}/myUsers')
        .doc(page.email)
        .delete();
    await isAddedFriendBefore(page.email);
    notifyListeners();
  }
}
